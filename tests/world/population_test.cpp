...
{
   public:
    // using statements are used for implicit template parameter propagation of
    // T
   ...
   protected:
    ...
    void SetUp() override { heapPop = new Population(cells); }
...
using TestTypes =
    ::testing::Types<entity::Plant, entity::Carnivore, entity::Herbivore>;
...
{
    typename TestFixture::Entity entity =
        typename TestFixture::Entity(1, 20, 20);
...
TYPED_TEST(PopulationTest, StackAddWorks_ManyEntitiesAdded)
{
    ...
    EXPECT_EQ(this->stackPop.idToIndexMap[this->stackPop.iterContainer[10].id],
              10);
}
...
{
    typename TestFixture::Entity* entity =
        new typename TestFixture::Entity(1, 20, 20);
...
TYPED_TEST(PopulationTest, HeapAddWorks_ManyEntitiesAdded)
{
    ...
    // idtoindexmap should find correct index
    EXPECT_EQ(this->heapPop->idToIndexMap[this->heapPop->iterContainer[10].id],
              10);
...
TYPED_TEST(PopulationTest, StackRemoveWorks_RemoveOne)
{
    ...
    // check if last entity is swapped to replace the removed entity in
    // itercontainer (id:100)
...
// check if last entity is swapped to replace the removed entity in
// itercontainer see comments above for explanation of expected values
