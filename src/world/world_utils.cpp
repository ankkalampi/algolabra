...
void generateTerrain(World &world, int amountWater, int amountRock,
                     int amountSand, int amountGrass) {

  // create perlin generator
  noise::module::Perlin perlin =
      terrain::generatePerlinGenerator(PERLIN_FREQUENCY, PERLIN_PERSISTENCE,
                                       PERLIN_LACUNARITY, PERLIN_OCTAVE_COUNT);
...
void createStartingPlants(World &world, int numberPlants) {
  ...
  for (int i = 0; i < numberPlants; ++i) {
    ...
    world.plants.insert(
        {cellIndex, new entity::Plant(world.runningId, 20, 20)});
    world.runningId++;
    // std::cout << "creating new plant in cell: x: " << x << ", y: " << y << ",
    // cellIndex: " << cellIndex << std::endl; std::cout << "size of plants map:
    // " << world.plants.size() << std::endl;
    rounds++;
  }

  // std::cout << "size of plants map: " << world.plants.size() << std::endl;
  // std::cout << "rounds: " << rounds << " numberPlants: " << numberPlants <<
  // std::endl;
  return;
}
